...
class DDPM:
    ...
    def learned_range_var(self, t, model_var_values):
        ...
        # print(model_var_values[0][0][0][0], frac[0][0][0][0])
    ...
    def p_mean_variance(self, x_t, t, num_images):
        ...
        if epsilon_theta.shape[1] == 6 or self.hparams["learn_sigma"]:
            epsilon_theta, model_var_values = self.half_channel(epsilon_theta)
    ...
    def p_sample_loop_ddpm(self, num_images, return_all_t=False, denorm=False, initial_noise=None):
        ...
        if return_all_t:
            if denorm:
                ...
                return self.get_pixel(step_footprint)
        else:
            if denorm:
                return self.denomalize(x_t)
            else:
                return self.get_pixel(x_t)
        # return self.denomalize(x_t)
    ...
    def half_channel(self, model_output):
