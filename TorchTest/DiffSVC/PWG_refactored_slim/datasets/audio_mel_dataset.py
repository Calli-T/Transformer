...
class AudioMelDataset(Dataset):
    ...
    def __init__(
        ...
        """
        # find all of audio and mel files_for_gen
        ...
        if audio_length_threshold is not None:
            ...
            if len(audio_files) != len(idxs):
                logging.warning(
                    "Some files_for_gen are filtered by audio length threshold "
        ...
        if mel_length_threshold is not None:
            ...
            if len(mel_files) != len(idxs):
                logging.warning(
                    "Some files_for_gen are filtered by mel length threshold "
        ...
        # assert the number of files_for_gen
        assert len(audio_files) != 0, f"Not found any audio files_for_gen in ${root_dir}."
        assert len(audio_files) == len(mel_files), (
            f"Number of audio and mel files_for_gen are different ({len(audio_files)} vs"
            f" {len(mel_files)})."
        )
        if self.use_local:
            assert len(audio_files) == len(local_files), (
                f"Number of audio and local files_for_gen are different ({len(audio_files)} vs"
                f" {len(local_files)})."
            )
        if self.use_global:
            assert len(audio_files) == len(global_files), (
                f"Number of audio and global files_for_gen are different ({len(audio_files)} vs"
                f" {len(global_files)})."
            )
...
class AudioMelF0ExcitationDataset(Dataset):
    ...
    def __init__(
        ...
        assert len(audio_files) == len(f0_files), (
            f"Number of audio and f0 files_for_gen are different ({len(audio_files)} vs"
            f" {len(f0_files)})."
        )
        assert len(audio_files) == len(excitation_files), (
            f"Number of audio and excitation files_for_gen are different ({len(audio_files)} vs"
    ...
    def __init__(
        ...
        if self.use_local:
            assert len(mel_files) == len(local_files), (
                f"Number of audio and local files_for_gen are different ({len(mel_files)} vs"
                f" {len(local_files)})."
            )
        if self.use_global:
            assert len(mel_files) == len(global_files), (
                f"Number of audio and global files_for_gen are different ({len(mel_files)} vs"
                f" {len(global_files)})."
            )
...
class MelF0ExcitationDataset(Dataset):
    ...
    def __init__(
        ...
        """
        # find all of the mel files_for_gen
        mel_files = sorted(find_files(root_dir, mel_query))
        f0_files = sorted(find_files(root_dir, f0_query))
        excitation_files = sorted(find_files(root_dir, excitation_query))
