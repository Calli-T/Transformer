...
class COCODataset(Dataset):
    ...
    def _load_data(self):  # 이미지들에 박스랑 클래스 정보 가져와서 처리함
        ...
        for _id in self.coco.imgs:
            file_name = self.coco.loadImgs(_id)["file_name"]
...
# 경로수정할것
train_dataset = COCODataset("pass", train=True, _transform=transform)
test_dataset = COCODataset("pass", train=False, _transform=transform)
