...
classifier = SentenceClassifier(_n_vocab=n_vocab, _hidden_dim=hidden_dim, _embedding_dim=embedding_dim,
                                _n_layers=n_layers, pretrained_embedding=getWord2VecInitEmbedding())
...
for epoch in range(epochs):
    train(classifier, train_loader, criterion, optimizer, "cpu", interval)
    test(classifier, test_loader, criterion, "cpu")
